...
body = ""
x = 0
text_queue = list(Utils.unformat(text).upper())
while len(text_queue) > 3:
    string = ''.join(text_queue[:3])
    if string in colours.keys():
        if x >= line_length-3:
            p = 3-(x-(line_length-3))
            body += "<span style='background-color: hsl(%s, 100%%, 50%%);'>%s</span>" % (colours[string], string[:p]+"<br>"+string[p:])
            x -= line_length
        else: body += "<span style='background-color: hsl(%s, 100%%, 50%%);'>%s</span>" % (colours[string], string)
        del text_queue[:3]
        x += 3
    else:
        body += text_queue.pop(0)
        if string[0] in uc: x += 1
    if x >= line_length:
        body += "<br>"
        x = 0
...
""" % body
print(html)
with open("coloured.html", "w") as html_file:
